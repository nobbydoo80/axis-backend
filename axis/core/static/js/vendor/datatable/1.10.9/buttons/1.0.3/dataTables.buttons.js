...
$.extend( Buttons.prototype, {
	...
	_buildButton: function ( config, collectionButton )
	{
		...
		var button = $('<'+buttonDom.tag+'/>')
			...
			.on( 'click.dtb', function (e) {
				...
				if ( ! button.hasClass( buttonDom.disabled ) && config.beforeAction ){
					config.beforeAction.call( dt.button( button ), e, dt, button, config );
				}
	...
	_resolveExtends: function ( conf )
	{
		...
		while ( conf && conf.extend ) {
			...
			// FIXME: Added by Michael 10/19/2015. Waiting on PR for
			// before and after actions.
			if(conf.afterAction){
				var _originalAction = conf.action;
				conf.action = function(e, dt, button, config){
					_originalAction(e, dt, button, config);
					conf.afterAction(e, dt, button, config);
				}
			}
		}
