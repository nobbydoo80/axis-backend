"""test_top_company_stats.py: """

__author__ = "Naruhito Kaide"
__date__ = "01/10/2022 13:56"
...
class CustomerHirlTopBuildersStatsTests(ApiV3Tests):
    @classmethod
    def setUpTestData(cls):
        super(CustomerHirlTopBuildersStatsTests, cls).setUpTestData()
    ...
    def test_builders_for_mf_grouped_by_unit_count(self):
        EEPProgram.objects.update(slug="ngbs-mf-new-construction-2020-new", is_multi_family=True)

        url = reverse_lazy("api_v3:home_status-customer-hirl-top-builder-stats")
        ...
        self.assertEqual(
            response.data["builders_for_mf_grouped_by_unit_count"],
            [
                (f"{self.registration.builder_organization.name}", 1),
                (f"{self.registration2.builder_organization.name}", 1),
            ],
        )

    def test_builders_for_sf_grouped_by_count(self):
        EEPProgram.objects.update(slug="ngbs-mf-new-construction-2020-new", is_multi_family=False)
        ...
        self.assertEqual(
            set(response.data["builders_for_sf_grouped_by_count"]),
            {
                (f"{self.registration.builder_organization.name}", 1),
                (f"{self.registration2.builder_organization.name}", 1),
            },
