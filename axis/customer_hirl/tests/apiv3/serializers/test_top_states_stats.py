...
class CustomerHirlTopStatesStatsTests(ApiV3Tests):
    @classmethod
    def setUpTestData(cls):
        super(CustomerHirlTopStatesStatsTests, cls).setUpTestData()
    ...
    def test_states_for_mf_grouped_by_unit_count(self):
        EEPProgram.objects.update(slug="ngbs-mf-new-construction-2020-new", is_multi_family=True)

        url = reverse_lazy("api_v3:home_status-customer-hirl-top-states-stats")
        ...
        self.assertEqual(
            response.data["states_for_mf_grouped_by_unit_count"],
            [
                (self.phoenix_city.county.state, 1),
                (self.midlothian_city.county.state, 1),
            ],
        )

    def test_states_for_sf_grouped_by_count(self):
        ...
        self.assertEqual(
            response.data["states_for_sf_grouped_by_count"],
            [
                (self.phoenix_city.county.state, 1),
                (self.midlothian_city.county.state, 1),
            ],
