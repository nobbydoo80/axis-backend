...
.run(function($http, Actions, RegionService){
  ...
  Actions.addPostMethodToType('save', ['hirl_builder_agreement_documents'], function(regionObject){
    reload(['hirl_builder_agreement', 'hirl_builder_enrollment']);
    return regionObject;
  });

  /* Debug */
  addTransitionMethod('debug_reset');
  ...
  addTransitionMethod('coi_expire');
  addTransitionMethod('expire');

  addForceSetStateMethod('approved');
  addForceSetStateMethod('insurance_received');
  addForceSetStateMethod('verified');
  addForceSetStateMethod('countersigned');
  addForceSetStateMethod('coi_expired');
  addForceSetStateMethod('expired');

  addApiMethod('ensure_blank_document', ['hirl_builder_agreement', 'hirl_builder_enrollment']);
  addApiMethod('update_docusign_status', ['hirl_builder_agreement', 'hirl_builder_enrollment']);
  addApiMethod('resend_email', []);

}).controller('BuilderAgreementEnrollmentFormController', function($scope, $http, $timeout, $window, $location){
